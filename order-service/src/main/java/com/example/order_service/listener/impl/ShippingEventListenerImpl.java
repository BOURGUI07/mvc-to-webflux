...
public class ShippingEventListenerImpl implements ShippingEventListener {
    ...
    public Mono<Void> handle(ShippingEvent.Ready shippingEvent) {
        return service.handleReadyShipping(ShippingMapper.fromReadyToShippingDTO().apply(shippingEvent));
    }

    @Override
    public Mono<Void> handle(ShippingEvent.Declined shippingEvent) {
        return service.handleFailedShipping(ShippingMapper.fromDeclinedToShippingDTO().apply(shippingEvent));
    }

    @Override
    public Mono<Void> handle(ShippingEvent.Cancelled shippingEvent) {
        return service.handleCancelledShipping(ShippingMapper.fromCancelledToShippingDTO().apply(shippingEvent));
    }

    @Override
    public Mono<Void> handle(ShippingEvent.Scheduled shippingEvent) {
        return service.handleScheduledShipping(ShippingMapper.fromScheduledToShippingDTO().apply(shippingEvent));
    }
}
